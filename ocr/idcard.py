from utils.tools import union_rbox
import re
...
class IDCard:
    ...
    def __init__(self, ocr_result):
        self.ocr_result = union_rbox(ocr_result, 0.2)
        self.N = len(self.ocr_result)
        self.idcard_info = {}
        self.full_name()
        self.sex()
        self.birthday()
    ...
    def full_name(self):
        ...
        for i in range(self.N):
            txt = self.ocr_result[i]['text'].replace(' ', '')
            txt = txt.replace(' ', '')
            ##匹配身份证姓名
            res = re.findall("姓名[\u4e00-\u9fa5]{1,4}", txt)
            if len(res) > 0:
                name['姓名'] = res[0].replace('姓名', '')
                self.idcard_info.update(name)
                break
